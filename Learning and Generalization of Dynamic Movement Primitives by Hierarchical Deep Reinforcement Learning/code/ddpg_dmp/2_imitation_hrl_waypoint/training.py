...
def check_sub_done(y, waypoint, cur_step_num):
    if cur_step_num == waypoint[0] - 1 and (y - waypoint[1])**2 < 1e-4:
        return 1
    else:
        return 0
        ...
        for epoch in range(nb_epochs):
            for cycle in range(nb_epoch_cycles): # Episode
                ...
                for t_rollout in range(dmp.n_step): # step
                    ...
                    r = 10*r1 + 0.001*r2 + 0.0*r3 + 10*r_w
                    ...
                    print('\rEpoch: {} | Ep: {} | Step: {} | R: {} | Y: {} | CS: {}'.format(\
                                        epoch, cycle, t_rollout,episode_reward, cur_y[0], obs*std_X + mean_X), end='')
