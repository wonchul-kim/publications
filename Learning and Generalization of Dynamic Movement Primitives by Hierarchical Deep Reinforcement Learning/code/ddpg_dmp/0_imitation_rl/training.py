...
def get_reward(y, dy, ddy, target, targetT, cur_step_num):
...
def check_done(y, target, targetT, cur_step_num):
    if cur_step_num == targetT and (y - target)**2 < 1e-4:
        return 1
    else:
        return 0
        ...
        for epoch in range(nb_epochs):
            for cycle in range(nb_epoch_cycles): # Episode
                ...
                for t_rollout in range(dmp.n_step): # step
                    # Predict next action.
                    canonical_traj.append(obs*std_X + mean_X)
                    ...
                    r1, r2, r3 = get_reward(cur_y, cur_dy, cur_ddy, path[-1], dmp.n_step - 1, t_rollout)
                    r = 100*r1 + 0.01*r2 + 0.0*r3
                    ...
                    print('\rEpoch: {} | Ep: {} | Step: {} | R: {}'.format(epoch, cycle, t_rollout,episode_reward), end='')

                
                # print(np.squeeze(canonical_traj))
                # print('===================================')
                # print(np.squeeze(force_traj))
                # print('===================================')
                # print(np.squeeze(f_target))
                # Episode done.
