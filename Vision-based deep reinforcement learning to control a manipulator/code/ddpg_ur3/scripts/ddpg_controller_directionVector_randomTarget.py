...
MODEL_PATH = '/home/icsl/catkin_ws/src/ddpg_ur3/scripts/Models'
...
class UR3_ARM:
    ...
    def reset(self, num_episode):
        ...
        global states, endPosition, TARGET
        direction_x = getSign(endPosition[0] - TARGET[0])
        direction_y = getSign(endPosition[1] - TARGET[1])
        direction_z = getSign(endPosition[2] - TARGET[2])
...
def parse_args():
    ...
    # boolean_flag(parser, 'isTrain', default=True)
...
def run(#env_id, 
    ...
    memory = Memory(limit=int(1e6), action_shape = (6,), observation_shape = (4,))
...
def train(env, nb_epochs, nb_epoch_cycles, eval_display_on, #render_eval, 
    ...
    agent = DDPG(actor, critic, memory, (4, ), (6, ),
        gamma=gamma, tau=tau, normalize_returns=normalize_returns, normalize_observations=normalize_observations,
        batch_size=batch_size, action_noise=action_noise, param_noise=param_noise, critic_l2_reg=critic_l2_reg,
        actor_lr=actor_lr, critic_lr=critic_lr, enable_popart=popart, clip_norm=clip_norm,
        reward_scale=reward_scale)
    ...
    with U.single_threaded_session() as sess:
        ...
        for epoch in range(nb_epochs):
            ...
            for cycle in range(nb_epoch_cycles):
                ...
                for t_rollout in range(nb_rollout_steps):
                    ...
                    print("\rRoll {}/{} @ Cycle{}/{} @ Episode {}/{} ||| R: {} ||| {} ||{}".format(t_rollout + 1, 
                        nb_rollout_steps, cycle + 1, nb_epoch_cycles, epoch, nb_epochs, episode_reward, TARGET, endPosition), end="")                  
            ...
            # Save Model
            if saveModel == True and epoch%3 == 0 and epoch !=0:
                saver.save(sess, MODEL_PATH + '/model-' + str(epoch) + '.cptk')
                print("<<<<< Saved Model >>>>>")
