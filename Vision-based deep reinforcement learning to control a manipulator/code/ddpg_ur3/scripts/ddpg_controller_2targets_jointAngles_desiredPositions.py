...
class UR3_ARM:
    ...
    def step(self, action, flag):
        ...
        if flag == True:
            current_state += (TARGET2[0], TARGET2[1], TARGET2[2])
        else:
            current_state += (TARGET1[0], TARGET1[1], TARGET1[2])

        return current_state, reward, done, flag
    ...
    def get_reward(self, end_position, action, flag):
        ...
        reward1 = -1*distance1 
        reward2 = -1.5*distance2 
        reward3 = - 0.1*np.linalg.norm(action)
...
def run(#env_id, 
    ...
    memory = Memory(limit=int(1e6), action_shape = (6,), observation_shape = (9,)) ############################################################
...
def train(env, nb_epochs, nb_epoch_cycles, eval_display_on, #render_eval, 
    ...
    agent = DDPG(actor, critic, memory, (9, ), (6, ), #################################################################################
        gamma=gamma, tau=tau, normalize_returns=normalize_returns, normalize_observations=normalize_observations,
        batch_size=batch_size, action_noise=action_noise, param_noise=param_noise, critic_l2_reg=critic_l2_reg,
        actor_lr=actor_lr, critic_lr=critic_lr, enable_popart=popart, clip_norm=clip_norm,
        reward_scale=reward_scale)
