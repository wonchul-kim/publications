...
DT_ANGLE = 2.0 # while initializing
...
class UR3_ARM:
    ...
    def reset(self):
        ...
        current_states += (TARGET1[0], TARGET1[1], TARGET1[2])
    ...
    def step(self, action, flag):
        ...
        # states += (flag, )
        if flag == True:
            states += (TARGET2[0], TARGET2[1], TARGET2[2])
        else:
            states += (TARGET1[0], TARGET1[1], TARGET1[2])

        return states, reward, done, flag

    def get_reward(self, end_position, action, flag):
        # print(end_position, action)
        distance1 = np.sqrt((self.target1[0]-end_position[0])**2 + (self.target1[1]-end_position[1])**2 + (self.target1[2]-end_position[2])**2)
        distance2 = np.sqrt((self.target2[0]-end_position[0])**2 + (self.target2[1]-end_position[1])**2 + (self.target2[2]-end_position[2])**2)
...
def run(#env_id, 
    ...
    if rank == 0:
        ...
        if evaluation:
            ...
            eval_env = UR3_ARM(TARGET1, TARGET2)
            # if gym_monitor and logdir:
    ...
    memory1 = Memory(limit=int(1e6), action_shape = (6,), observation_shape = (9,)) ############################################################
    memory2 = Memory(limit=int(1e6), action_shape = (6,), observation_shape = (9,)) ############################################################
    ...
    train(env=env, eval_env=eval_env, param_noise=param_noise,
        action_noise=action_noise, actor=actor, critic=critic, memory1=memory1, memory2=memory2, **kwargs)
...
def train(env, nb_epochs, nb_epoch_cycles, eval_display_on, #render_eval, 
    ...
    popart, gamma, clip_norm, nb_train_steps, nb_rollout_steps, nb_eval_steps, batch_size, memory1, memory2,
    ...
    agent = DDPG(actor, critic, memory1, memory2, (9, ), (6, ), #################################################################################
        gamma=gamma, tau=tau, normalize_returns=normalize_returns, normalize_observations=normalize_observations,
        batch_size=batch_size, action_noise=action_noise, param_noise=param_noise, critic_l2_reg=critic_l2_reg,
        actor_lr=actor_lr, critic_lr=critic_lr, enable_popart=popart, clip_norm=clip_norm,
        reward_scale=reward_scale)
        ...
        for epoch in range(nb_epochs):
            ...
            for cycle in range(nb_epoch_cycles):
                ...
                for t_rollout in range(nb_rollout_steps):
                    ...
                    epoch_qs.append(q)
                    agent.store_transition(obs, action, r, new_obs, done, flag)
                    obs = new_obs

                    print("\rRoll {}/{} @ Cycle{}/{} @ Episode {}/{} ||| R: {} ||| {} || {} || {}".format(t_rollout + 1, 
                        nb_rollout_steps, cycle + 1, nb_epoch_cycles, epoch, nb_epochs, episode_reward, endPosition, flag, obs[7]), end="")                  
                ...
                for t_train in range(nb_train_steps):
                    ...
                    if flag == True:
                        if memory2.nb_entries >= batch_size and t % param_noise_adaption_interval == 0:
                            distance = agent.adapt_param_noise()
                            epoch_adaptive_distances.append(distance)
                    else:
                        if memory1.nb_entries >= batch_size and t % param_noise_adaption_interval == 0:
                            distance = agent.adapt_param_noise()
                            epoch_adaptive_distances.append(distance)

                    cl, al = agent.train(flag)
